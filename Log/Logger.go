...
func _console(s string, levelname string, flag _FORMAT, calldepth int) {
	cc := 31
	if strings.Compare(levelname, "[FATAL]") == 0 {
		cc = 31
	} else if strings.Compare(levelname, "[ERROR]") == 0 {
		cc = 35
	} else if strings.Compare(levelname, "[WARN]") == 0 {
		cc = 33
	} else if strings.Compare(levelname, "[INFO]") == 0 {
		cc = 32
	} else if strings.Compare(levelname, "[DEBUG]") == 0 {
		cc = 34
	} else {
		cc = 37
	}
	if flag != FORMAT_NANO {
		...
		fmt.Printf("\x1b[%dm%s\x1b[0m", cc, &buf)
	} else {
