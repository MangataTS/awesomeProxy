...
var _ AsCache.PeerGetter = (*httpGetter)(nil)
...
type ReverseProxy struct {
	...
	self string
	// basePath，作为节点间通讯地址的前缀，默认是 /ascache/，
	...
	httpGetters map[string]*httpGetter // 映射远程节点与对应的 httpGetter。每一个远程节点对应一个 httpGetter，keyed by e.g. "http://10.0.0.2:8008"
...
func (p *ReverseProxy) Set(peers ...string) {
	p.mu.Lock()
	defer p.mu.Unlock()
...
func (p *ReverseProxy) PickPeer(key string) (AsCache.PeerGetter, bool) {
	p.mu.Lock()
	defer p.mu.Unlock()
	if peer := p.peers.Get(key); peer != "" && peer != p.self {
		Log.Debug("Pick peer ", peer)
		return p.httpGetters[peer], true
	}
	return nil, false
}

var _ AsCache.PeerPicker = (*ReverseProxy)(nil)
